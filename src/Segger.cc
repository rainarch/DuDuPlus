...
Segger::Segger(SegPipe *p1, SymOptions *p2){
	ppipe = p1;
	poptions = p2;
	params.Init(p1->feat_list.GetSize());
}

void Segger::Train(string &trainfile){
...
void Segger::TrainingIter(string &trainfile, int iter){
    ifstream feat_rd;
...
int Segger::LoadModelNew(string &model_name){
    ifstream model_rd;
...
void Segger::SaveModelNew(string &model_name){
	ofstream model_wd;
...
void Segger::SaveModel(string &model_name){
...
int Segger::LoadModel(string &model_name){
...
void Segger::Test(string &testfile){
    time_t start_time, end_time;
    start_time = time(NULL);
	string type = "SEG";
		...
		*/
		...
		if(d.size() > 0){
			for(int k = 0; k < K && k < d.size(); ++k){
				...
				//cerr << d[k].parse_tree << endl;
				//ppipe->SeqTagCheck(d[k].parse_tree);
			}
...
void Segger::TestOneSent(Instance &instance, vector<ResultItem > &d){
	    int length = instance.word_items.size();
        // Get production crap.
        vector<FeatureVector> fvs;//[.length][.length][2]
        fvs.reserve(length*ppipe->types_num*ppipe->types_num);
...
void Segger::TestOneSent(Instance &instance){
	vector<ResultItem > d;
	TestOneSent(instance, d);
	if(d.size() > 0){
		//only k = 0;
		...
		vector<WordItem> newword_items;
		newword_items.reserve(tag_items.size());
		newword_items.push_back(instance.word_items.at(0));
		string words;
    	for(int i = 0; i < tag_items.size()-1; i++) {
        	string seqtag = tag_items[i].c_str();
	//		cerr << instance.word_items.at(i+1).form << seqtag << endl;
			if(i != 0 && (seqtag == "S" || seqtag == "B"))
				words += " ";
			words += instance.word_items.at(i+1).form;
    	}
		vector<string> vw;
		boost::split(vw, words, boost::is_any_of(" "));
		for(int i = 0; i < vw.size(); i++){
			WordItem word_item;
			word_item.form = vw.at(i);
			newword_items.push_back(word_item);
	//		cerr << word_item.form  << endl;		
		}
		//
		newword_items.push_back(instance.word_items.at(instance.word_items.size()-1));
		instance.word_items = newword_items;
   }
